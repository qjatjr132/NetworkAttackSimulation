        ...
        if self.ptr != 0:
            for i in range(self.ptr):
                if (np.array_equal(self.s_buf[i], s) == True) and \
                    ...
                    print(
                        f"[+] s={np.where(s != 0)[0]}, a=[{a}], next_s={np.where(next_s != 0)[0]}, r={r}, done={done}")
                    return
                else:
                    # print(f"[+]buf_i={i}")
                    continue
...
class DoubleDQNAgent:
    ...
    def __init__(self,
        ...
        # logger setup
        self.logger = SummaryWriter('./runs/double_dqn_Defender/')
    ...
    def optimize(self):
        ...
        with torch.no_grad():
            ...
            target_s_a_prime = target_q_val_raw.gather(1, a_prime.unsqueeze(1))
            target_s_a_prime = target_s_a_prime.squeeze()
            target_s_a_prime = r_batch + self.discount * (1 - d_batch) * target_s_a_prime
        if self.steps_done % self.log_print_count == 0 & self.PRINT_ON == True:
            print(f"[+] target : {target_s_a_prime}")

        # calculate loss
        # kjkoo : (23-0806) ~ TD? (temporal difference)
        loss = self.loss_fn(q_vals, target_s_a_prime)
...
if __name__ == "__main__":
    ...
    dqn_agent = DoubleDQNAgent(env, verbose=args.quite, **vars(args))

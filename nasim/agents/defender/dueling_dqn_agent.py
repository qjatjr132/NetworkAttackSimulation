...
This is by no means a state of the art implementation of DuelingDQN, but is designed
    ...
    def get_action(self, x, steps_done, log_print_count, PRINT_ON):
        with torch.no_grad():  # auto-grad off
            ...
            if steps_done % log_print_count == 0 & PRINT_ON == True:
                print(f"[+] @get_action: fwd_x(q_vals-DuelingDQN out) :{fwd_x[0][0:5]}, max:{fwd_x.max(1)[0]}")
...
class DuelingDQNAgent:
    ...
    def __init__(self,
        ...
        # This DuelingDQN implementation only works for flat actions
        assert env.flat_actions
        self.verbose = verbose
        if self.verbose:
            print(f"\nRunning DuelingDQN with config:")
            pprint(locals())
        ...
        self.num_actions = self.env.defender_action_space.n
        self.obs_dim = self.env.D_observation_space.shape

        # logger setup
        self.logger = SummaryWriter('./runs/dueling_dqn_Defender/')
    ...
    def get_epsilon_adaptive(self):
        ...
        # kjkoo : (23-0706)
        # input : o, state, St -> DuelingDQN 입력
        # output : #action, At -> DuelingDQN 출력
...
if __name__ == "__main__":
    ...
    dqn_agent = DuelingDQNAgent(env, verbose=args.quite, **vars(args))
    ...
    # kjkoo : (23-0808)
    #   - dqn_agent policies 저장 (DuelingDQN parameters(weight)

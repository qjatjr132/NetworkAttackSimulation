...
class DQN(nn.Module):
    def __init__(self, input_dim, hidden_size, n_actions):
        super(DQN, self).__init__()
        self.fc1 = NoisyLinear(input_dim[0], hidden_size[0])
        self.fc2 = NoisyLinear(hidden_size[0], hidden_size[0])
        self.fc3 = NoisyLinear(hidden_size[0], n_actions)
...
class NoisyDQNAgent:
    ...
    def __init__(self,
        ...
        self.policy_dqn = DQN(self.obs_dim,
                       hidden_sizes,
    ...
    def select_action(self, state):
        ...
        if sample > self.epsilon:
            with torch.no_grad():
                return self.policy_dqn(state).max(1)[1].view(1, 1).item()
        else:
            return torch.tensor([[random.randrange(self.num_actions)]], dtype=torch.long).item()

    def save(self, save_path):
        self.policy_dqn.save_DQN(save_path)

    def load(self, load_path):
        self.policy_dqn.load_DQN(load_path)
    ...
    def get_egreedy_action(self, o, epsilon, steps):
        ...
        if random_value > epsilon:
            ...
            # kjkoo : (23-0712)
            # kjkoo : (23-0806)
            action_tensor, Q_val = self.policy_dqn.get_action(o, self.steps_done, self.log_print_count, self.PRINT_ON)
    ...
    def optimize(self):
        ...
        q_vals_raw = self.policy_dqn(s_batch)
        q_vals = q_vals_raw.gather(1, a_batch).squeeze()  # action(a)의 Q값
        next_state_values = self.target_dqn(next_s_batch).max(1)[0].detach()
        # kjkoo
        if self.steps_done % self.log_print_count == 0 & self.PRINT_ON == True:
        ...
        loss = self.loss_fn(q_vals.unsqueeze(1), expected_state_action_values.unsqueeze(1))
        # kjkoo
        if self.steps_done % self.log_print_count == 0 & self.PRINT_ON == True:
    ...
    def print_dqn_parameters(self):
        print(f"\nDQN-weight: \n{self.policy_dqn.layers[0].weight}")
        print(f"\nDQN-weight-grad: \n{self.policy_dqn.layers[0].weight.grad}")
        print(f"===============")
        return

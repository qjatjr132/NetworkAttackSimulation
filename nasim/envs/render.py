...
class Viewer:
    ...
    def render_d_readable_state(self, state):
        """Print a readable tabular version of observation to stdout
    ...
    def _construct_table_from_list_of_dicts(self, l):
        ...
        for d in l:
            ...
            table.add_row(row)
        return table
        # kjkoo : (23-0530) attack_pps를 반영하기 위해, key(feature) 이름을 줄인다.

        # kjkoo : (23-0530) attack_pps 이름이 길어서 짧게 줄임 => 가독성 높이기
        #   - (24-0808) pps-v2 (24-0730) 버전으로 업데이트
    def _shorten_table_name(self, headers):
        #
        attack_pps_dict = {
            "Compromised": "Compr",
            "Reachable": "Reach",
            "Discovered": "Discv",
            # ====================================
            "Value": "SHVal",  # Sensitive Host
            "Discovery Value": "DHVal",  # Discovered Host
            # ====================================
            "privilege": "priv",
            # os/service
            "os_type": "os_t",
            "os_status": "os_s",
            "eternalblue": "eblue",
            "eternalblue_s": "eblue_s",
            "eternalblue_v": "eblue_v",
            "firewall": "fw",
            "firewall_s": "fw_s",
            "firewall_v": "fw_v",
            # file
        ...
        # print(f"attack_pps_dict: \n{attack_pps_dict}")
        pps_keys = list(attack_pps_dict.keys())
        headers_short = list()
        for name in headers:
            if name in pps_keys:
                headers_short.append(attack_pps_dict[name])
            else:
                headers_short.append(name)
        return headers_short

    # kjkoo
    #   - (24-0628) 상태 테이블 정렬
    #   - (24-0808) 특징 추가
    def _custom_sort_render(self, hlist):
        host_status_list = ['Address', 'Compr', 'Reach', 'Discv', 'SHVal', 'DHVal']
        # 13 종
        service_list = ['av', 'av_s', 'av_v',
                        'db', 'db_s', 'db_v',
                        'eblue', 'eblue_s', 'eblue_v',
                        'fw', 'fw_s', 'fw_v',
                        'ftp', 'ftp_s', 'ftp_v',
        ...
        if 'priv' in hlist:
            headers_short_sorted.append('priv')
        if 'os_t' in hlist:
            headers_short_sorted.append('os_t')
        if 'os_s' in hlist:
            headers_short_sorted.append('os_s')
        if 'os_v' in hlist:
    ...
    # kjkoo
    #   - (24-0628)
    def _sort_table_name(self, headers_short_list):  # 24-0628
        # print(f'>> headers_short_list: {headers_short_list}')
        headers_short_sorted = self._custom_sort_render(headers_short_list)
        return headers_short_sorted

    # kjkoo : (23-0530) attack_pps를 반영하기 위해, key(feature) 이름을 줄인다.
    #   - (24-0628) sort state table
    def _construct_table_from_list_of_dicts_with_attack_pps(self, l):
        headers = list(l[0].keys())
        headers_short = self._shorten_table_name(headers)
        headers_short_sorted = self._sort_table_name(headers_short)  # 24-0628
        table = PrettyTable(headers_short_sorted)
        for d in l:
            # row = [str(d[k]) for k in headers]
            row = list()
            for k in headers:

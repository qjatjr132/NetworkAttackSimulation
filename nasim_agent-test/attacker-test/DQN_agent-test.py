...
import nasim
from nasim.agents.attacker.dqn_agent_attacker import DQNAgent_Attacker

if __name__ == "__main__":

    env = nasim.make_benchmark('tiny-etri-alp-pps2-r220',fully_obs=True, flat_actions=True, flat_obs=True)

    dqn_agent_attacker = DQNAgent_Attacker(env, lr=0.0,
                                           ...
                                           gamma=0.0,
                                           hidden_sizes=[64, 64],
                                           target_update_freq=1000, verbose=True, )

    dqn_agent_attacker.load('./policy/dqn_agent.pt')
    ...
    while episode < episodes:
        ...
        while steps < step_limit and (not done_a):
            ...
            next_o_a, r_a, done_a, env_step_limit_reached_a, _ = dqn_agent_attacker.env.step(a_a, "attacker", 'None')
        ...
        if max_reward <= ak_episode_return:
            ...
            with open("action_list/DQN/max_reward_DQN_Action_list.txt", "w") as file:
                # 리스트의 각 요소를 파일에 쓰기
        ...
        if min_steps >= steps:
            min_steps = steps
            with open("action_list/DQN/min_step_DQN_Action_list.txt", "w") as file:
                # 리스트의 각 요소를 파일에 쓰기
                for item in action_list:
                    file.write(str(item) + ",")
        ...
        # dqn_agent_attacker.save('./policy/dqn_agent.pt')

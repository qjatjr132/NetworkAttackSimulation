...
import nasim
from nasim.agents.attacker.noisydqn_agent_attacker import NoisyDQNAgent

if __name__ == "__main__":

    env = nasim.make_benchmark('tiny-etri-alpinelab',fully_obs=True, flat_actions=True, flat_obs=True)

    dqn_agent_attacker = NoisyDQNAgent(env, lr=0.001, training_steps=2000000, batch_size=32, replay_size=10000, final_epsilon=0.05,
                                           exploration_steps=5000, gamma=0.99, hidden_sizes=[64,64], target_update_freq=1000, verbose=True,)
    ...
    while episode < episodes:
        ...
        while (steps < step_limit) and (not done_a):
            #attacker
            a_a = dqn_agent_attacker.select_action(o)
